...
get '/callback' do
  ...
  if token
    ...
    user = User.find_or_create_by(username: user["screen_name"], token: params["oauth_token"], secret: params["oauth_verifier"])


  else
    logger.info "User didn't authorize us"
  end
...
post '/new' do
  ...
  User.tweet(session[:user], params[:tweet])
